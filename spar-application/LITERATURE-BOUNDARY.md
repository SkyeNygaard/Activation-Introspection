...
|---|---|
| Clustering of a hidden class predicts whether a four-shot interface can learn it ([16](../activation-introspection/notes/16-visible-rule-capacity.md)) | **Unclaimed, and it failed its own replication on 2026-08-12.** [notes/26](../activation-introspection/notes/26-someone-elses-rules.md) reran it on fourteen rules written by a model blind to the hypothesis: rank agreement 0.403 against notes/19's 0.785, permutation p = 0.078, and the usable half — positive separation means learnable, 8 for 8 — fell to 4 of 7. Nobody else claims it because it does not generalise. **Do not put it in an application** |
| **Abstention does not repair a trained introspective monitor** ([29](../activation-introspection/notes/29-can-abstention-recover-selectivity.md)) | **The strongest candidate in this repository, and the only one with a positive search result behind it.** Selective prediction — risk-coverage curves, letting a model decline to answer — is a mature subfield applied to question answering and hallucination. Searched 2026-08-12: **it has never been applied to a model's reports about its own internal states.** Every introspection paper reports accuracy and false-positive rate at full coverage only. This asks the deployment question instead, and answers it negatively: dropping the least-confident reports *narrows* the gap between concept-derived and matched arbitrary directions in trained reporters (0.059→0.013 and 0.099→0.019) while nearly doubling it in the untrained model (0.232→0.455). **Corrected 2026-08-14 — this previously read "concept-versus-noise gap".** The arbitrary directions are demonstrated before they are queried and have correct answers, so both sides of that gap are solvable tasks; the claim is not about noise. It also lands on a **named open problem** — [Introspection Adapters](https://arxiv.org/pdf/2604.16812) lists "abstention mechanisms" as one of three candidate fixes for the false-positive rate limiting their method. **Honest label: extension candidate, and the design is the contribution.** One model, one recipe, one strength cell, and a secondary analysis of artifacts frozen for another purpose. It does *not* test the DPO-refined adapters Anthropic actually proposes |
| Confidence ranks a model's own self-reports even when its mean confidence does not separate them ([29](../activation-introspection/notes/29-can-abstention-recover-selectivity.md)) | **Correction of [20](../activation-introspection/notes/20-comparator-tiers.md), not a novelty claim.** That AUROC beats a difference of means on a ceiling-squashed variable is standard; what is recorded here is that this repository got it wrong and how |
| The sensitivity/specificity trade-off in introspection training ([08](../activation-introspection/notes/08-sensitivity-specificity-tradeoff.md)) | **Never novelty-checked until 2026-08-12, and it is an extension.** Three of its four parts are prior art: base-model selectivity against norm-matched random vectors is [Lindsey](https://arxiv.org/abs/2601.01828); training raising sensitivity is in several papers; and "training makes introspective reports hallucinate things that are not there" is the opening limitation of [Introspection Adapters](https://arxiv.org/pdf/2604.16812). **Not found:** the two measured together as one trade-off, against a *matched-magnitude meaningless* control rather than an absent one. Extension, not new headline. See [PAPERS-REVIEWED.md](PAPERS-REVIEWED.md) |
