...
| A trained zero-demonstration reporter | 504 rows per run. V3 is the citable result: four seeds fixed in the protocol before any ran, pooled in `report_training_v3_seeds_summary.json`, trained target 0.927 range [0.833, 1.000], every seed passing every gate. V1 (loss restricted to the label logits; fails the verbalization gate under the current analyzer) and V2 (loss repaired, initialization unseeded, 0.583 from one uncontrolled draw) are retained as disclosed precursors. Regenerate with `make report-training-seeds-report` |
| A convention/strength training study | 4,608 rows per seed, three seeds. V2 is the citable result: protocol `f29b479d…be0f0`; raw `0ab9de44…2e97a1`, `ae4ff7ff…dfa210`, `f0005147…fb7e42`; runner `03e8ba2a…6c66`; pooled in `remap_training_v2_seeds_summary.json` with all four gates passing. V1 is the falsified precursor, retained with `gate set v1` and `all_gates_pass=false`; gate sets are keyed to the producing protocol so a later analyzer cannot re-judge an earlier artifact. Regenerate with `make remap-training-report` |
